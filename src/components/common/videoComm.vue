    ...
    <div class="multiple-video-show">
        ...
        <ul id="video-content" :class="rowNumStyle">
            <li class="win" ref="winLi" v-for="(item,index) in videoWinSelect" v-show="videoWinSelect[index].isShow"
                :key="index" :class="{'checked':index==checkedVideoWin}" @click="handleCheckVideoWin(index)"
                ...
                <div class="win-content" :id="item.winId" :data-win="item.winId" data-ctrl="close" data-loc="close"
                    ...
                    <video class="video" :id="item.videoId" width="640" height="480" autoplay></video>
                    <div class="dir-group" v-show="showInfo.dirShow[index]" v-for="(dir,d) in dirSelectData" :key="d">
                        <i class="dir" :class="dir.id" :data-type="dir.type" :title="dir.name" @mousedown="handleDirMouseDown(index,dir.type,dir.name)" @mouseup="handleDirMouseUp(index,dir.type,dir.name)"></i>
                    </div>
                    <div>
                        <div class="play-pause" :style="playBtnfontSize" v-show="showInfo.playPauseShow[index]"><i
                        ...
                        <div class="video-operate" v-show="showInfo.operateShow[index]">
                            <ul class="video-operate-ul">
                                <li class="float-left cam-name">{{showInfo.cameraName[index]}}</li>
                                <li class="float-right video-play" title="播放" @click.stop="handleOperBtn(0,index)" v-show="playState=='realPlay'?false:true">
                                    <i class="fa fa-play" aria-hidden="true"></i>
                                </li>
                                <li class="float-right video-pause" title="暂停" @click.stop="handleOperBtn(1,index)" v-show="playState=='realPlay'?false:true">
                                    <i class="fa fa-pause" aria-hidden="true"></i>
                                </li>
                                <li class="float-right video-close" title="关闭" @click.stop="handleOperBtn(2,index)">
                                    <i class="fa fa-times" aria-hidden="true"></i>
                                </li>
                                <li class="float-right video-cut" title="截图" @click.stop="handleOperBtn(3,index)" v-show="false">
                                    <a class="capture" href="javascript:;">
                                        <i class="fa fa-camera":style="showInfo.operBtnActiveColor[index][3]" aria-hidden="true"></i>
                                    </a>
                                </li>
                                <li class="float-right video-illegal" title="手动抓拍" @click.stop="handleOperBtn(4,index)">
                                    <a class="capture" href="javascript:;">
                                        <i class="fa fa-camera-retro" :style="showInfo.operBtnActiveColor[index][4]"  aria-hidden="true"></i>
                                    </a>
                                </li>
                                <li class="float-right video-ctrl" title="云台控制" @click.stop="handleOperBtn(5,index)">
                                    <a class="capture" href="javascript:;">
                                        <i class="fa fa-arrows" :style="showInfo.operBtnActiveColor[index][5]" aria-hidden="true"></i>
                                    </a>
                                </li>
                                <li class="float-right video-locate" title="3D定位" @click.stop="handleOperBtn(6,index)"v-show="false">
                                    <a class="capture" href="javascript:;">
                                        <i class="fa fa-location-arrow" :style="showInfo.operBtnActiveColor[index][6]" aria-hidden="true"></i>
                                    </a>
                                </li>
                            </ul>
                        </div>
...
<script>
    ...
    import {checkUserCameraPower} from '@/api/video'
        ...
        methods: {
            ...
            handleDblclickVideoWin(index) {
                ...
                if (this.winNum > 1) {
                    ...
                    console.log(this.videoWinSelect, "$$$$");
                } else {
...
//              passParam.$emit('openDrawer',{maskVisable:false,drawerVisable:true});
                            ...
                            checkUserCameraPower(params).then(res => {
                                if (res.data) {
                                    ...
                                    this.$message({message: '该用户没有对摄像机进行手动抓拍的权限', type: 'warning'})
                                }
                            })
                        }
                        break;
                    case 5:
                        ...
                        checkUserCameraPower(params).then(res=>{
                            if(res.data){
                                ...
                                this.$message({message: '该用户没有对摄像机进行云台控制的权限', type: 'warning'})
                            }
                        });
