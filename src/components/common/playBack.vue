...
<script>
    ...
    export default {
        props: ['playState', 'startTime', 'endTime', 'hoursPerRuler', 'btnShow', 'winCount', 'curWinCount', 'curRowCls'],
        data() {
            return {
                winSelectData: [
            ...
            handleClickBar(clickTime) {
                ...
                checkUserCameraPower(params).then(res=>{
                    if(res.data){
                        ...
                        this.$message({message: '该用户没有对摄像机进行录像播放的权限', type: 'warning'})
                    }
                });
            },
