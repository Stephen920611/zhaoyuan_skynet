                ...
                <el-row :gutter="20">
                    <el-col :span="8" style="height: 470px">
                        ...
                        <div class="role-list-content">
                            <el-card class="box-card" shadow="hover" v-for="(item,index) in viewArr" :key="index"
                                     @click.native="showId(item,index)" :class="activeClass === index ? 'active' : ''"
...
//    import playBack from '@/components/common/playBack.vue'
    ...
    export default {
        ...
        mounted() {
            ...
            this.getList();
            //不显示左侧列表
//            this.showId(this.videoTracingInfo, 0);
        },
        methods: {
            //获取列表数据
            getList() {
            ...
            //录像回放窗口
            playBackVideo(item) {
                if(item.operateTime && item.operateTime !== ''){
                    //前后取的秒数
