    ...
    <div class="app-container">
        <el-container>
            <el-aside width="350px" class="search-tree all-background" style="overflow: hidden;">
                <CommonSubtitle title="应急资源列表" :isBold="true"
            ...
            <el-main style="padding: 20px 20px 0 20px">
                    ...
                    <el-form :inline="true" :model="form" ref="form" class="form-inline-search" label-width="120px">
                        <el-row :gutter="20">
                            <el-col :span="this.isSmallBtn ? 12 : 8">
                                <el-form-item label="所属储备点：" prop="attribution" class="form-inline-search-item">
                                    <el-select v-model="form.storageId" class="filter-item"
                                               placeholder="请选择所属储备点">
                                        <el-option
                            ...
                            <el-col :span="this.isSmallBtn ? 12 : 8">
                                <el-form-item label="物资名称：" prop="name" class="form-inline-search-item">
                ...
                <el-card class="table-card">
                    ...
                    >
                        ...
                        <el-table-column label="物资名称" header-align="center" align="center">
                            <template slot-scope="{row}">{{row.name}}</template>
                        </el-table-column>
                        <el-table-column label="物资储备点名称" header-align="center" align="center">
                            <template slot-scope="{row}">{{row.storageName}}</template>
                        </el-table-column>
                        <el-table-column label="物资类别" header-align="center" align="center">
                            <template slot-scope="{row}">{{row.typeName}}</template>
                        </el-table-column>
                        <el-table-column label="规格型号" header-align="center" align="center">
                            <template slot-scope="{row}">{{row.model}}</template>
                        </el-table-column>
                        <el-table-column label="物资数量" header-align="center" align="center">
                            <template slot-scope="{row}">{{row.amount}}</template>
                        </el-table-column>
                        <el-table-column label="计量单位" header-align="center" align="center">
                            <template slot-scope="{row}">{{row.measure}}</template>
                        </el-table-column>
                    </el-table>
                    <!-- 分页控件 -->
        ...
        <div class="emergency-map-container">
            ...
            <div class="emergency-info">
                ...
                <div style="padding-left: 10px" v-if="!(JSON.stringify(detailInfo) == '{}' || !detailInfo)" >
                    <el-row>
                        <el-col :span="12">
                            应急物资名称：{{detailInfo.name}}
                        </el-col>
                        <el-col :span="12">
                            物资数量：{{detailInfo.amount}}
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-col :span="12">
                            应急物资仓库名称：{{detailInfo.storageName}}
                        </el-col>
                        <el-col :span="12">
                            计量单位：{{detailInfo.measure}}
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-col :span="12">
                            物资类别：{{detailInfo.typeName}}
                        </el-col>
                        <el-col :span="12">
                            物资技术状况：{{detailInfo.technology}}
                        </el-col>
                    </el-row>
                    <el-row>
                        <el-col :span="12">
                            规格型号：{{detailInfo.model}}
                        </el-col>
                        <el-col :span="12">
                            物资其他情况说明：{{detailInfo.mark}}
                        </el-col>
                    </el-row>
                </div>
            </div>
        ...
        >
            <add-emergency-resources type="add" :companyInfo=companyInfo :detailInfo="detailInfo" :storageOptions="storageOptions"
                                     v-on:toggleHidden="toggleHidden"></add-emergency-resources>
        </el-dialog>

        <!--新增的dialog-->
        <el-dialog title="编辑" :visible.sync="editShow" v-if="editShow"  width="950px"
                   :close-on-click-modal="false" append-to-body @close="closeDialog">
            <add-emergency-resources type="edit" :companyInfo=companyInfo :detailInfo="detailInfo" :storageOptions="storageOptions"
                                     v-on:toggleHidden="toggleHidden"></add-emergency-resources>
        </el-dialog>
...
<script>
    ...
    import AddEmergencyResources from './components/add-emergency-resources.vue'
//    import EditEmergencyResources from './components/edit-emergency-resources.vue'
    import elDragDialog from '@/directive/el-drag-dialog'

    import {fetchAreaTree} from '@/api/courseMonitor/alarmSynthesis';
    import {
        getByCode,
        fetchEmergencyMaterialPage,
        delEmergencyMaterial,
        fetchEmergencyStoragList,
        emergencyMaterialSaveOrUpdateBounds
    ...
    export default {
        ...
        components: {Pagination, CommonSubtitle, googleMapView,AddEmergencyResources,elDragDialog,ToggleGoogleMap},
...
//                let params = this.form.companyId !== null ? {companyId:this.form.companyId} : {};
                fetchEmergencyStoragList({}).then(res => {
            ...
            getList() {
                ...
                let obj = {
                    ...
                    storageId: self.form.storageId
                };
                this.listLoading = true;
                fetchEmergencyMaterialPage(obj).then(res => {
...
//                    this.getStorage();
            ...
            delEmergency() {
                ...
                if (JSON.stringify(this.detailInfo) == '{}' || !this.detailInfo) {
                    this.$message({
                        message: '请选择需要删除的应急资源',
                        type: 'warning'
                    });
                    return
                }
                this.$confirm('此操作将删除该应急资源, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    delEmergencyMaterial({id: this.detailInfo.hasOwnProperty('id') ? this.detailInfo.id : null}).then(res => {
                        self.$message({
                            showClose: true,
                            message: '删除成功！',
                            type: 'success'
...
//                    this.$message({message: '暂未标注，请先标注位置信息！', type: 'warning'})
            ...
            saveOrUpdateBounds(params) {
                emergencyMaterialSaveOrUpdateBounds(params).then(res => {
                    this.$message({
                        showClose: true,
                        message: '保存成功！',
                        type: 'success'
