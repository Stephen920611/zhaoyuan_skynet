        ...
        <div class="tree-wrap" :class="{'tree-wrap-not-search':!isSearch}">
            ...
            <div id="perTreeMenu" v-if="tmDisplay" class="tree_menu" :style="{...rightMenu}">
                <ul v-for="(menu,index) in rightMenus" :key="index">
                    <li class="menu-li" :id="menu.id"><!--<i :class="menu.icon"></i>-->{{menu.name}}</li>
                </ul>
            </div>
        </div>
    </div>
...
//            this.$refs.tree.setCurrentKey(null);
                if(node.type && node.type.indexOf('depart')>-1){//有下级节点
...
//              alert('ssss')
            ...
            handleSelectSearchNode(node){
                ...
                if(curNode.type.indexOf('depart')>-1){
...
//                  reqParam = qs.stringify(reqParam);
            ...
            rightClick(e,data,node,comp){
                //暂时这么写，看看后续有什么好的方法
                if(data.type=='camera'){
                    this.$emit('updateRightMenu','camera');
                }else{
                    this.$emit('updateRightMenu','depart');
                    //右键点击分组时，展开下级节点
                    this.getNextNodeList(data);
                }
                ...
                document.onclick=function(ev){
                    ...
                    if(ev.target.className == "menu-li"){
                        ...
                        self.$emit('openDialog',true,type,data)
                    }
                }
            },
        }
