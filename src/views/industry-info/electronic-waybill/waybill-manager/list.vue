...
<template>
    <div class="app-container">
                    ...
                    <el-container class="container-wrap">
                        ...
                        <el-main>
                            ...
                            >
                                ...
                                <el-table-column label="运输货物名称" header-align="center" align="center" min-width="40">
                                    <template slot-scope="{row}">{{row.cargoNames.join(',')}}</template>
                                </el-table-column>
                                <el-table-column label="数量" header-align="center" align="center" min-width="20">
                                    <template slot-scope="{row}">{{row.cargoNumbers.join(',')}}</template>
                                </el-table-column>
        ...
        <!--查看的dialog-->
        <el-dialog title="电子运单详情" :visible.sync="dialogVisible"  width="1200px" :close-on-click-modal="false" append-to-body v-elDragDialog>
            <see-detail :editParamsInfo="editParamsInfo"></see-detail>
        </el-dialog>
        <!--新增的dialog-->
        <el-dialog title="新增电子运单" :visible.sync="addWaybillShow"  width="1200px" :close-on-click-modal="false" append-to-body v-elDragDialog @open="openAddWayBillDialog" @close="closeAddWayBillDialog">
            <add-waybill v-on:showHiddenAdd="showHiddenAdd" ref="addWayBill" :companyId="selectedCompanyId"></add-waybill>
        </el-dialog>
    </div>
</template>
    ...
    export default {
        ...
        data() {
            return {
                ...
                showOperateBtn:false,  //显示“新增”和“作废”按钮，默认不显示
                driverNameArr:[],  //驾驶员姓名的数组
                selectedCompanyId:null,//选中公司树节点
                selectedAreaId:null,//选中区域树节点
            };
        },
        watch: {
            filterText(val) {
...
//                    this.showOperateBtn = true;  //测试使用
            ...
            addWaybill(){
                if(this.selectedCompanyId){
                    ...
                    this.$message({message: '请选择公司！', type: 'warning'});
                }
            },
            //查看详情
            seeDialog(row){
...
//                    companyId:self.userInfo.companyId,
                ...
                eleWaybill(obj).then(res => {
                    res.data.list.map(item=>{
                        ...
                        item.cargoNames = [];
                        item.cargoNumbers = [];
                        item.chemicalList.map(jtem=>{
                            item.cargoNames.push(jtem.goodsName);
                            item.cargoNumbers.push(jtem.weight);
                        })
                    })
