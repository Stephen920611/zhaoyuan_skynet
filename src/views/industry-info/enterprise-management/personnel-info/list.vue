    ...
    export default {
        ...
        data() {
            return {
                ...
                selectedCompanyId:null,//选择中公司树节点
                selectedAreaId:null,//选择中区域树节点
                defaultProps: {
                    children: 'nodes',
                    label: 'text'
                },
        ...
        methods: {
          ...
          /*  //关闭“新增”弹窗，清除表单数据
            closeAddDialog(){
                this.$nextTick(()=>{
                    this.$refs.addPersonnel.resetForm('ruleForm');
                });
            },*/
            //table选择按钮
            ...
            //新增
            addPersonnelShow(){
                if (this.selectedCompanyInfo.hasOwnProperty('id')) {
                    console.log(this.selectedCompanyInfo,'this.selectedCompanyInfo');
                    this.$store.dispatch("industryInfo/enterpriseManagement/personnelInfo/setCompanyDetail", {personnelCompanyDetail: this.selectedCompanyInfo});
                    this.addShow = true
                } else {
