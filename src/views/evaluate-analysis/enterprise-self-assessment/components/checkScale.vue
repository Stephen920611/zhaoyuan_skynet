                    ...
                    >
                        <el-table-column label="文件名称" type="index" align="center" width="130px">
                            <template slot-scope="{row}">
                                {{Number(row.grandParentInfo.appraiseTotalScore) > 0 ? row.fileName + '（' + row.grandParentInfo.appraiseTotalScore + '分）' : row.fileName}}
                            </template>
                        </el-table-column>
                        <el-table-column label="考核内容" header-align="center" align="center" min-width="20%">
                            <template slot-scope="{row}">
                                {{Number(row.parentInfo.appraiseTotalScore) > 0 ? row.checkContent + '（' + row.parentInfo.appraiseTotalScore + '分）' : row.checkContent}}
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column label="是否涉及" header-align="center" align="center" width="100px">
                            <template slot-scope="{row}">
                                <el-checkbox :disabled="formStatus === 'view'" v-model="row.isSelected" @change="(val)=>selectedChange(val,row)"></el-checkbox>
                            </template>
                        </el-table-column>
                        <el-table-column label="频次" header-align="center" align="center" min-width="14%"
                                         v-if="this.clickId !== 1">
                            <template slot-scope="{row}">
                                <el-input-number v-if="Number(row.info.appraiseType) === 3" v-model="row.rate"
                                                 controls-position="right" @change="(newValue,oldValue)=>rateChange(newValue,oldValue,row)" :min="0"
                                                 style="width: 100%;"
                                                 :disabled="formStatus === 'view'"
                                ></el-input-number>
                            </template>
                        </el-table-column>
                        <el-table-column label="得分" header-align="center" align="center" width="100px"
                                         v-if="this.clickId !== 1">
                            <template slot-scope="{row}">{{clickId === 2 ? Number(row.parentInfo.showTotalScore) : Number(row.grandParentInfo.showTotalScore)}}</template>
                        </el-table-column>
                    </el-table>
                </el-main>
                ...
                tree.map((val, index) => {
                    ...
                    if (val.nodes) {
                        ...
                        LastTreeLevelContent.push({
                            ...
                            checkContent: parentInfo.appraiseDes,
                            fileName: grandParentInfo.appraiseDes,
                        });
                    }
                });
            ...
            calcShowTotalScore(){
                ...
                this.treeData.map( val => {
                    ...
                    if(val.appraiseType === '0'){
                        //clickId为2的是获取所有儿子节点，所有不为2的是获取孙子节点
                        if(this.clickId !== 2){
