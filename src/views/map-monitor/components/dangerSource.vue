        ...
        <div class="search-wrap">
            ...
            <el-row>
                <el-col :span="14" class="search-item">
                    <el-input v-model="form.name" placeholder="请输入重大危险源名称" @keyup.enter.native="handleSearch"></el-input>
                </el-col>
        ...
        <div class="content" v-scrollBar>
            ...
            <div class="line-item" :class="{'active':index==selectedItemIndex}"  v-for="(item,index) in list" :key="index" v-else  @click="handleSelect(index,item)">
                <div class="base-info">
                    ...
                    <p>港口危险货物名称：{{item.hazard}}</p>
                </div>
                <div class="btn-wrap">
                    <p><span @click="seeDetail">详情<i class="fa fa-chevron-circle-right"></i></span></p>
                </div>
            </div>
        </div>
        <!-- 分页控件 -->
...
<script>
    import seeDetail from '../../hazard-installations/hazard-management/hazard-management-company/components/see-detail.vue'
    import Pagination from '@/components/Pagination'
    import elDragDialog from '@/directive/el-drag-dialog'
    import utility from '@/utils/ext.utility'
    import mapShowFunc from '@/utils/mapShowFunc'
    import {queryDangerSource} from '../../../api/map'
    import {getCompanyDetail,getByCode} from '../../../api/industry'
    export default {
        ...
        data() {
            return {
                ...
                seeDetailFlag:false, //窗口显示
            }
        },
        directives: {elDragDialog},
        components: {Pagination,seeDetail},
        mounted() {
            this.form.areaId = parseInt(this.areaId);
            this.dictionary();
            this.dangerSourceList()
        },
        methods: {
            //字典查询
            dictionary() {
            ...
            dangerSourceList() {
                ...
                queryDangerSource(obj).then(res => {
                    this.list = res.data;
//                    this.total = res.data.total;
            ...
            handleSelect(index,data) {
                ...
                //在地图上定位所在位置
                const boundsAry = data.boundsAry;
                if(boundsAry && boundsAry.length>0){
                    ...
                    boundsAry.map((bounds,index)=>{
                        ...
                        //定位--放大级别--弹出气泡窗口
                        this.locMarkerOpenInfoWindow(data.id,'dangerSource',true);
                    })
                }else{
