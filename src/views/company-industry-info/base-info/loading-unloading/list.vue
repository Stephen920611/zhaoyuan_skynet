    ...
    <div class="map-container">
        ...
        <div class="filter-container">
            <el-form :inline="true" :model="form" class="form-inline-search">
                <el-row>
                    <el-col :span="5">
                        <el-form-item label="装卸区名称：" class="form-inline-search-item">
                            <el-input
                                v-model.trim="form.name"
                                placeholder="请输入装卸区名称"
                                class="filter-item"
                                style="width: 95%;"
        ...
        <!--新增的dialog-->
        <el-dialog title="新增" v-if="addShow" :visible.sync="addShow"  width="500px" :close-on-click-modal="false" append-to-body>
            <add-loading v-on:showHiddenAdd="showHiddenAdd"></add-loading>
        </el-dialog>
        <!--编辑的dialog-->
        <el-dialog title="编辑" v-if="editShow" :visible.sync="editShow"  width="500px" :close-on-click-modal="false" append-to-body>
            <edit-loading v-on:showHiddenEdit="showHiddenEdit" :loadingData="list[selectedItemIndex]"></edit-loading>
        </el-dialog>
    </div>
</template>
<script>
    ...
    import {getLadeArea,ladeArea} from '@/api/industry'
    import CommonSubtitle from '@/components/CommonSubtitle'
    import elDragDialog from '@/directive/el-drag-dialog'
    import addLoading from './components/add-loading.vue'
    import EditLoading from './components/edit-loading.vue'
    export default {
        ...
        components: {googleMapView, switchItem, dropDown,CommonSubtitle,addLoading,EditLoading},
        created() {
            this.getList();
        ...
        methods: {
            ...
            handleSelect(index) {
                ...
                if(!curTunnel.location || curTunnel.location=='[]'){
                    this.$confirm('该装卸区没有位置信息, 是否标注?', '提示', {
                        ...
                        mapShowFunc.mapVariable.addMarkerTag='loadPlace32&'+curTunnel.id+'&'+curTunnel.name;
                ...
                //画隧道的开始节点和结尾处节点
                const firstMarker = mapShowFunc.addMarker(this.map, curTunnel.id, JSON.parse(curTunnel.location)[0].lat, JSON.parse(curTunnel.location)[0].lng, 'loadPlace32', curTunnel.name, "device", true, true,()=>{});
                ...
                if(isActive){
                    ...
                    if(code=='save'){
                        ...
                        if (this.TunnelsArr.length!=0) {
                            this.$confirm('装卸区已有位置信息, 是否重新标注?', '提示', {
                                confirmButtonText: '确定',
                                cancelButtonText: '取消',
                                type: 'warning'
                            }).then(() => {
                        ...
                        //标注操作
                        if (curTunnel.location || this.saveFlag) {
                            this.$message({
                                type: 'warning',
                                message: '该装卸区已标注'
                            });
            ...
            getList(){
                let self = this;
                this.loading = true;
                getLadeArea(this.form).then(res => {
                    self.list=res.data.list;
                    self.total=res.data.total;
                    this.loading = false;
                    /*  //当选择区域的时候，将该区域所有的点都显示出来
                      if(this.form.areaId){
                          let marker=null;
                          self.list.forEach(item => {
                              //画隧道的开始节点和结尾处节点
                              marker = mapShowFunc.addMarker(this.map, item.id, item.lat, item.lng, 'loadPlace32', item.name, "device", true, true,()=>{});
