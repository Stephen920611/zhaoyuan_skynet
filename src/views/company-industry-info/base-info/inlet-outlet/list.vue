    ...
    <div class="map-container">
        ...
        <div class="filter-container">
            <el-form :inline="true" :model="form" class="form-inline-search">
                <el-row>
                    <el-col :span="5">
                        <el-form-item label="进出口道名称：" class="form-inline-search-item">
                            <el-input
                                v-model.trim="form.name"
                                placeholder="请输入进出口道名称"
                                class="filter-item"
                                style="width: 95%;"
        ...
        <el-dialog title="新增" :visible.sync="addShow"  width="500px" :close-on-click-modal="false" append-to-body>
            <add-inorout v-on:showHiddenAdd="showHiddenAdd"></add-inorout>
        </el-dialog>
        <!--编辑的dialog-->
        <el-dialog title="编辑" :visible.sync="editShow"  width="500px" :close-on-click-modal="false" append-to-body>
            <edit-inorout v-on:showHiddenEdit="showHiddenEdit" :inOutdata="list[selectedItemIndex]"></edit-inorout>
        </el-dialog>
    </div>
</template>
<script>
    ...
    import {getBayonet,basicBayonet} from '@/api/industry'
    import CommonSubtitle from '@/components/CommonSubtitle'
    import elDragDialog from '@/directive/el-drag-dialog'
    import addInorout from './components/add-inorout.vue'
    import EditInorout from './components/edit-inorout.vue'
    export default {
        name: 'MapMonitorIndex',
        directives: {elDragDialog},
        components: {googleMapView, switchItem, dropDown,CommonSubtitle,addInorout,EditInorout},
        data() {
            return {
                map: null,
        ...
        methods: {
            ...
            saveLocation() {
                ...
                console.log(this.markerLatLng,'this.markerLatLng');
                basicBayonet(obj).then(res => {
                    self.$message({
                        showClose: true,
                        message: '操作成功！',
                        type:'success'
            ...
            handleSelect(index) {
                ...
                if(!curTunnel.lat || !curTunnel.lng){
                    this.$confirm('该进出口道没有位置信息, 是否标注?', '提示', {
                        ...
                        mapShowFunc.mapVariable.addMarkerTag='bayonet32&'+curTunnel.id+'&'+curTunnel.name;
                ...
                //画隧道的开始节点和结尾处节点
                const firstMarker = mapShowFunc.addMarker(this.map, curTunnel.id, curTunnel.lat, curTunnel.lng, 'bayonet32', curTunnel.name, "device", true, true,()=>{});
                ...
                if(isActive){
                    ...
                    if(code=='save'){
                        ...
                        if (this.TunnelsArr.length!=0) {
                            this.$confirm('进出口道已有位置信息, 是否重新标注?', '提示', {
                                confirmButtonText: '确定',
                                cancelButtonText: '取消',
                                type: 'warning'
                            }).then(() => {
                  ...
                  /*  //当选择区域的时候，将该区域所有的点都显示出来
                    if(this.form.areaId){
                        ...
                        self.list.forEach(item => {
                            //画隧道的开始节点和结尾处节点
                            marker = mapShowFunc.addMarker(this.map, item.id, item.lat, item.lng, 'bayonet32', item.name, "device", true, true,()=>{});
