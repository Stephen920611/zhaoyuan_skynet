...
<script>
    ...
    import {bridgeSaveOrUpdate} from '@/api/industry'
    ...
    export default {
        ...
        data() {
            return {
                ...
                rules: {
                    name: [
                        { required: true, message: '请输入隧道名称', trigger: 'change' }
                    ],
                    areaId: [
        ...
        created(){
            ...
            for(let key of keys){
                this.ruleForm[key] = this.bridgeData[key];
            }
            this.getAllRegions();
        },
        watch:{
        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        bridgeSaveOrUpdate(this.ruleForm).then(res => {
                            this.$message({
                                showClose: true,
                                message: '编辑成功！',
                                type: 'success'
