...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            handleDraw(code) {
                if(this.list == null){
                    this.$message({message:'请选择需要绘图的公司',type:'warning'});
                    return
                }
                let drawType = "";
...
//                        mapShowFunc.searchByRect();		//画矩形
                    ...
                    case 'mark':
                        /*一次打点*/
                        mapShowFunc.mapVariable.addMarkerTag='company32&'+this.list.id+'&'+this.list.companyName;
                        passParam.$on('marker',(tunnelMarker,tunnelLine)=>{
                            this.addCompanyMarker.push(tunnelMarker[0])
                        });
                        //时刻监控位置坐标
                        passParam.$on('latLng',(latLngInfo)=>{
                            this.latLngArrays.push(latLngInfo);
                        });
            ...
            //修改位置，重新绘制，清空当前选择的公司所有的点位信息
            clearDrawFigure(){
                if(this.list == null){
                    this.$message({message:'请先从列表中选择一个公司',type:'warning'});
                    return
                }
                this.list.boundsAry.map((bounds,boundIndex)=>{
                    //清空选择的当前公司的点
                    let companyMarkers = this.companyHash.getValue(this.list.id+ '-' + boundIndex);
                    if(companyMarkers){
                        companyMarkers.setMap(null);
                        if (companyMarkers.label) {
                            companyMarkers.label.setMap(null);
                        }
                    }
                    //清空选择的当前公司的区域
                    let shape = this.companyShapeHash.getValue(this.list.id+ '-' + boundIndex);
                    if(shape){
                        shape.setMap(null);
                    }
                });
            ...
            clearSelectedMarker(){
                ...
                //清空所有公司的点
                let companyMarkers = this.companyHash.getValues();
                companyMarkers.map(item=> {
                    item.setMap(null);
                    if (item.label) {
                        item.label.setMap(null);
                    }
