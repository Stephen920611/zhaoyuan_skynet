<template>
    <div class="app-container">
            ...
            <el-main style="padding: 10px;">
                <div class="text item">
                    ...
                    <el-container class="container-wrap">
                        ...
                        <el-header class="header-wrap" style="height: 100px" v-scrollBar>
                            <el-row style="width: 100%;margin-bottom: 10px">

                                <el-col :span="7" class="search-wrap">
                                    <div class="input-suffix">
                                        <el-col style="margin-top: 7px;width:100px"><span class="label-name" style="float: right">运单编号：</span>
                                        </el-col>
                                        <el-col :span="14">
                                            <el-input
                                                style="float: left"
                                ...
                                <el-col :span="7" class="search-wrap">
                                    <div class="input-suffix">
                                        <el-col style="margin-top: 7px;width:80px;"><span class="label-name">车牌号：</span>
                                        </el-col>
                                        <el-col :span="15">
                                            <el-input
                                                style="float: left"
                                ...
                                <el-col :span="24" class="search-wrap">
                                    <div class="input-suffix">
                                        <el-col  style="margin-top: 7px;width: 100px">
                                            <span class="label-name" style="float: right">状态：</span>
                                        </el-col>
                        ...
                        <el-main>
                            ...
                            <el-row style="width: 100%;margin-bottom: 10px">
                                <el-col :span="3" class="btns" style="float: left">
                                    <el-button type="primary" size="medium"
                                               ...
                                               class="filter-item"
                                               @click="addWaybill">新 增</el-button>
                                    <el-button type="danger" size="medium"
                                               @click="deleteWaybill"
                                               icon="el-icon-delete">作 废</el-button>
                                </el-col>
                            </el-row>
                            ...
                            >
                                ...
                                <el-table-column label="驾驶员姓名" header-align="center" align="center">
                                    <template slot-scope="{row}">{{row.driverName}}</template>
                                </el-table-column>
                                <el-table-column label="要货地点" header-align="center" align="center">
                                    <template slot-scope="{row}">{{row.transportDestination}}</template>
                                </el-table-column>
                                <el-table-column label="运输目的地" header-align="center" align="center">
                                    <template slot-scope="{row}">{{row.receiverCompanyName}}</template>
                                </el-table-column>
                                <el-table-column label="危险货物名称" header-align="center" align="center">
                                ...
                                <el-table-column label="应急联系电话" header-align="center" align="center">
                                    <template slot-scope="{row}">{{row.emergencyTelephone}}</template>
                                </el-table-column>
                                <el-table-column label="运单状态" header-align="center" align="center">
        ...
        <!--查看的dialog-->
        <el-dialog title="电子运单详情" :visible.sync="dialogVisible"  width="65%" :close-on-click-modal="false" append-to-body>
            <see-detail :editParamsInfo="editParamsInfo"></see-detail>
        </el-dialog>
        <!--新增的dialog-->
        <el-dialog title="新增电子运单" :visible.sync="addWaybillShow"  width="60%" :close-on-click-modal="false" append-to-body>
            <add-waybill v-on:showHiddenAdd="showHiddenAdd"></add-waybill>
        </el-dialog>
        <!--处置的承运方dialog-->
        <el-dialog title="承运方" :visible.sync="transportShow"  width="60%" :close-on-click-modal="false" append-to-body>
            <task-assignment v-on:showHiddenTask="showHiddenTask" :editParamsInfo="editParamsInfo"></task-assignment>
        </el-dialog>
        <!--装载企业dialog-->
        <el-dialog title="装载企业" :visible.sync="loadingShow"  width="60%" :close-on-click-modal="false" append-to-body>
            <loading-enterprise v-on:showHiddenLoading="showHiddenLoading" :editParamsInfo="editParamsInfo"></loading-enterprise>
        </el-dialog>
        <!--装载企业dialog-->
        <el-dialog title="卸货企业" :visible.sync="dumpShow"  width="60%" :close-on-click-modal="false" append-to-body>
            <dump-truck v-on:showHiddenDump="showHiddenDump" :editParamsInfo="editParamsInfo"></dump-truck>
        </el-dialog>
    </div>
</template>
...
<script>
    ...
    import {eleWaybillDelete} from '@/api/industry'
    ...
    import loadingEnterprise from './components/loading-enterprise'
    import dumpTruck from './components/dump-truck'
    export default {
        name: 'OrderTrackList',
        components: {dumpTruck,loadingEnterprise,taskAssignment,AddWaybill, SeeDetail, Pagination,CommonSubtitle},
        ...
        data() {
            return {
                ...
                checkedStatus:"",
                state:["运单发起","车辆分配","等待装车","运输途中","待卸车","运输完成"],
                filterText: '',
                treeData: [],
                defaultProps: {
                    children: 'nodes',
        ...
        methods: {
            ...
            deleteWaybill() {
                ...
                if(!(this.currentRow.consignorCompanyId == this.userInfo.companyId && this.currentRow.wayBillStatus == 1)){
                    this.$message({message: '该运单不是自己发起的，不可删除！', type: 'warning'});
                    return false
                }
                //符合删除条件后，可删除
                let obj={id:this.currentRow.id};
                this.$confirm('此操作将删除该电子运单, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    eleWaybillDelete(obj).then(res => {
