        ...
        <div class="container">
            <el-row class="qualify-item margin-d-10" v-for="(item,index) in existData" :key="index">
                <el-col :span="isDisabled?24:23" class="fa-border padding-t-10">
                    <use-certificate-form :ref="'form_'+index" :isDisabled="isDisabled" :carInfo="carInfo" :data="existData[index]"  role="edit" > </use-certificate-form>
                </el-col>
            ...
            <el-row class="qualify-item margin-d-10" v-for="(item,index) in qualifys" :key="'info_'+index">
                <el-col :span="isDisabled?24:23" class="fa-border padding-t-10">
                    <use-certificate-form :ref="'form_'+index" :isDisabled="isDisabled" :carInfo="carInfo" role="add"> </use-certificate-form>
                </el-col>
                <el-col :span="1" class="margin-t-15 padding-lr-20" v-show="!isDisabled">
            ...
            delQualify(index){
                ...
                if(this.existData[index] && this.existData[index].id){
                    this.$confirm('此操作将永久删除该条数据, 是否继续?', '提示', {
                        ...
                        deleteCarLiquidGaxLicense({id:this.existData[index].id}).then(res=>{
                            this.$message({
                                showClose: true,
                                message: '删除成功！',
                                type: 'success'
