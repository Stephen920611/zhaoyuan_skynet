google.maps.__gjsload__('drawing_impl', function(_) {
    ...
    var T7 = function(a, b) {
        ...
        c.unbind("map");
        a.j = null;
        //console.log(c.getRadius());//这里是圆和矩形的
        //console.log(a.S.S());
        if (a.S.S()=='circle') {
            $.post('add.php',{type:'circle',lng:c.getCenter().lng(),lat:c.getCenter().lat(),radius:c.getRadius(),fillColor:c.fillColor,strokeColor:c.strokeColor,strokeWeight:c.strokeWeight,fillOpacity:c.fillOpacity,strokeOpacity:c.strokeOpacity},function  (data) {
                // console.log(data);
            });
        };
        if(a.S.S()=='rectangle'){
            var rectangle_data=[];
            console.log(c.fillColor);
            rectangle_data.push([c.getBounds().O.O,c.getBounds().j.j,c.fillColor,c.strokeColor,c.strokeWeight,c.fillOpacity,c.strokeOpacity]);
            rectangle_data.push([c.getBounds().O.j,c.getBounds().j.O,c.fillColor,c.strokeColor,c.strokeWeight,c.fillOpacity,c.strokeOpacity]);
            $.post('add.php',{type:'tangle',data:rectangle_data},function  (data) {
                console.log(data);
            });
            rectangle_data=null;
        }
    ...
    var a8 = function(a) {
       ...
       for (var i = 0; i < b.getPath().j.length; i++) {
           tangle_data.push([b.getPath().j[i].lng(),b.getPath().j[i].lat(),b.fillColor,b.strokeColor,b.strokeWeight,b.fillOpacity,b.strokeOpacity]);
       };
        if (a.P) {
            //poly
           $.post('add.php',{type:'poly',data:tangle_data},function  (data) {
               console.log(data);
           });
        } else{
            //line
            $.post('add.php',{type:'line',data:tangle_data},function  (data) {
               console.log(data);
           });
        };
        tangle_data=null;
    ...
    V7.prototype.P = function(a) {
        ...
        //这是里是标注的
        $.post('add.php',{type:'little_marker',lng:a.lng(),lat:a.lat()},function  (data) {
            console.log(data);
        });
